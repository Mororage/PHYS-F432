...
\end{rap}
\begin{rap}
    Le \emph{tenseur de Faraday} (également appelé le \emph{field strength tensor}) est un tenseur complètement anti-symmétrique donné par
    \begin{equation}
...
\end{rap}
\begin{exerc}
    Trouvez à partir de (\ref{eq:Farady intro}) la relation du tenseur de Faraday en fonction des champs $(\vect{E},\vect{B})$.
\end{exerc}
\cutebreak
...
\begin{exerc}
    \label{exercice:maxwell1}
    Montrez que la seconde équation de (\ref{eq:maxwell ch1}) est identiquement satisfaite par (\ref{eq:Farady intro}). En effet, la description par un potentiel implique directement les équations de Maxwell homogènes ($\vect{\nabla} \times \vect{E} + \pd_t \vect{B}= 0$ et $\vect{\nabla} \cdot \vect{B} = 0$). 
\end{exerc}
...
\end{rmk}
\begin{rmk}
    La propriété \ref{post:PE2} permet d'expliquer pourquoi l'interaction gravitationnelle est observable à des échelles macroscopiques malgré son extrême faiblesse :
    \begin{equation}
...
\begin{theoremframe}
    \begin{propri}[Principe d'équivalence, première version]
        En tout point de l'espace-temps, il est possible de choisir un système de coordonnées tel que localement autour de ce point, les lois de la physique prennent la même forme qu'en l'absence de gravitation dans un référentiel inertiel.\footnote{Weinberg.}
    \end{propri}
\end{theoremframe}
...
\begin{theoremframe}
    \begin{propri}[Principe d'équivalence, deuxième version]
        Localement, aucune expérience ne permet de distinguer un champ de gravitation et une accélération uniforme.\footnote{Hartle.}
    \end{propri}
\end{theoremframe}
Notons que la notion \emph{locale} utilisée ici peut se définir d'une manière mathématiquement précise, sur laquelle nous reviendrons plus tard. Pour le moment, il est seulement important de retenir que l'équivalence entre gravitation et accélération n'est valable que dans des régions suffisamment restreintes de l'espace-temps, sans quoi les inhomogénéités du champ gravitationnel feront que l'effet sur des \emph{particules-test} ou objets environnants sera différent d'un référentiel accéléré uniformément.
...
\begin{exerc}
    Démontrez la relation $\dfrac{\Delta \lambda}{\lambda_1} = \dfrac{\Delta v}{c}$ dans la limite non-relativiste.
\end{exerc}
...
En conclusion, dans un champ de gravitation, la période du signal n'est pas la même au pied de la tour que en haut de la tour. Le temps s'écoule plus rapidement lorsqu'on s'éloigne d'une source gravitationnelle\footnote{cf. \emph{Interstellar} lorsqu'iels se rapprochent du trou noir.}. Il s'agit d'un \textit{redshift gravitationnel}. Cet effet a été mesuré pour la première fois par Pound et Rebka (1959).
