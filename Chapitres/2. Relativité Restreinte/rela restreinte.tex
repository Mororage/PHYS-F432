...
\begin{itemize}
    \item Représentation 2-dimensionnelle. Préférer autant que possible des coordonnées à valeurs bornées\footnote{Par exemple, $\theta \in [0,\pi[$ plutôt que $u \in \R$. Ce n'est néanmoins pas toujours possible (comme par exemple pour $\mathrm{AdS}_n$).}.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.55\linewidth}
        ...
        \caption{Espace (conformellement) de Minkowski (bleu) dans les coordonnées $(U,V)$. Note : les coordonnées $(T,R)$ sont ici changés d'un facteur $2$.}
        \label{fig:UV Mink}
    \end{subfigure}
    \quad \quad
    \begin{subfigure}[b]{0.32\linewidth}
        \includegraphics[width=\linewidth]{Chapitres/2. Relativité Restreinte/Images/Diagramme Penrose.pdf}
        \caption{Diagramme de Penrose de $\R^{1,3}$.\\
        \,}
        \label{fig:Penrose Mink}
    \end{subfigure}
    \caption{Diagrammes de l'espace-temps de Minkowski. Tout point sur ceux-ci représente une sphère $\mathcal{S}^2$. Sur le diagramme de Penrose ont été indiquées des courbes causales à $r=\text{cste}$ (bleu) et des courbes à $t=\text{cste}$ (rouge). Ont également été illustrés les cônes de lumière en deux points. Les quantités indiqués sur le bord sont définies ci-dessous.}
...
\begin{exerc}
    L'espace \emph{de Sitter} en 3 dimensions est un sous-espace maximalement symétrique de Minkowski et à courbure constante positive :
    \begin{align}
        \mathrm{dS}_3 = \{x \in \R^{1,3} \mid \eta_{\mu\nu} x^\mu x^\nu = l^2\}.
    \end{align}
    Construisez le diagramme de Penrose associé.
\end{exerc}
\section*{Références}
Coming soon...
\begin{center}
