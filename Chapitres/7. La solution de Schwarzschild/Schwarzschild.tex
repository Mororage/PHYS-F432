...
Einstein lui-même, après avoir publié ses équations en 1915, ne fut capable que d'en trouver des solutions approchées non-triviales (différentes de Minkowski pour $\Lambda = 0$). Il fut donc surpris lorsque \emph{Karl Schwarzschild}, en décembre 1915, lui adressa une lettre du front russe\footnote{il était engagé dans les troupes allemandes au cours de la première guerre mondiale, où il contracta une maladie qui causera sa mort en mai 1916.} lui annonçant avoir trouvé la première solution exacte non-triviale des équations d'Einstein du vide. Il s'agit d'une \emph{solution à symétrie sphérique}, qui correspond en bonne approximation au champ gravitationnel généré par un corps céleste comme le soleil ou la terre. Il s'agit d'une solution \emph{extérieure}, i.e. satisfaisant à $G_{\mu\nu} = 0$ : on considère que l'espace est vide autour d'un corps gravitant à symétrie sphérique.

En plus de son utilité \emph{pratique} (par exemple, pour déterminer les corrections relativistes aux équations de Newton pour le mouvement des planètes dans le système solaire), cette solution nous permettra de décrire des objets nouveaux d'un très grand intérêt : les \emph{trous noirs}. Ceux-ci seront étudié au prochain chapitre.

\section{La symétrie sphérique}
La symétrie sphérique en (3+1) dimensions est définie comme la symétrie $SO(3)$ spatiale (de la sphère $S^2$), i.e. les rotations ordinaires de l'espace euclidien à 3 dimensions. Cette symétrie joue un rôle important dans de nombreux problèmes physiques (mécanique classique : problème de Kepler, mécanique quantique : atome d'Hydrogène). La 2-sphère est définie par la contrainte
\begin{equation}
    S^2 = \{ (x,y,z) \in \R^3 \mid x^2 +y^2 +z^2 = 1 \}
\end{equation}
Et peut être paramétrisée\footnote{pour notre niveau de rigeur, presque partout.} par
\begin{align}
    \label{def:coordonnées sphériques}
    \begin{dcases}
...
\begin{align}
    \xi^{x} &= (-x\sin{\alpha} + y\cos{\alpha})\big |_{\alpha = 0} = y\\
    \xi^{y} &= (-x\cos{\alpha} - y\sin{\alpha})\big |_{\alpha = 0} = -x\\
    \xi^{z} &= 0
\end{align}
Qu'on peut écrire sous la forme 
\begin{equation}
    \xi = \xi^{\mu}\partial_{\mu} = y\partial_{x} - x\partial_{y} \equiv Z
\end{equation}
Similairement, on trouve pour des rotations autour des autres axes :
\begin{align}
    X \equiv z\partial_{y}-y\partial_{z} \\
    Y \equiv x\partial_{z} - z\partial_{x}\\
    Z \equiv y\partial_x - x \partial_y
\end{align}
Néanmoins, la convention standard que nous utiliserons s'écrit légèrement différemment :
\begin{align}
    \begin{dcases}
        R &\equiv Z \\
        S &\equiv - Y \\
    ...
    \end{dcases}
\end{align}
On souhaite exprimer ces vecteurs en coordonnées sphériques. Effectuons le calcul pour $R$, et laissons les deux autres comme exercice. On peut convertir $R$ par un calcul brut via la règle de la chaîne en coordonnées sphériques :
\begin{equation}
...
\end{equation}
En prenant la 2-sphère comme sous-ensemble de $\R^3$, nous pouvons calculer le dual de $R$ comme vecteur de $\R^3$ : $\tilde R = y \td x - y \td x$, ce qui donne en coordonnées sphériques :
\begin{align}
    \begin{dcases}
        R_x = \sin \theta \sin \varphi\\
        R_y = - \sin \theta \cos \varphi
    \end{dcases}
\end{align}
En transformant également la base $(\td x,\td y)$ en coordonnées sphériques, on obtient alors :
\begin{align*}
    ...
    &= - \sin^2{\theta}\, \td \varphi
\end{align*}
On peut alors effectuer la transformation inverse $R^{\mu} = g^{\mu \nu}R_{\nu}$ pour obtenir en coordonnées sphériques
\begin{align}
...
\begin{equation}
    R = -\partial_{\varphi}
\end{equation}
Similairement, on \footnote{Il s'est avéré qu'il y avait une erreur de signe sur la page Wikipédia des vecteurs de Killing, ce qui m'avait pas mal confusé... (c'est corrigé à présent).}
\begin{align}
    T &= + \sin \varphi \, \pd_\theta + \cot \theta \cos \varphi \, \pd_\varphi\\
    S &= - \cos \varphi \, \pd_\theta +\cot \theta \sin \varphi \, \pd_\varphi 
\end{align}
On peut vérifier que les trois vecteurs $R$, $S$ et $T$ sont bien des vecteurs de Killing de la métrique \ref{def:S2} de $S^2$), c'est-à-dire que 
\begin{equation}
    \label{eq:sym sphérique killing}
    \mathcal{L}_{\xi_A}g_{\mu \nu} = 0 \quad\text{ pour } \xi_A = (R, S, T)
\end{equation}
\begin{rmk}
    ...
    \end{align}
\end{rmk}
Les vecteurs $\xi_A$ satisfont à l'algèbre de $\mathfrak{so}(3)$ :
\begin{equation}
    [R,S]  = T ; \quad [S,T]  = R ; \quad [T,R]  = S
\end{equation}
ou encore 
\begin{equation}
    \label{eq:algèbre de so(3)}
    [\xi_A,\xi_B] = \epsilon_{ABC} \xi_C
\end{equation}
où $[\, \cdot \, , \, \cdot \, ]$ est le crochet de Lie.
\begin{rmk}
    Si $u$ et $v$ sont deux vecteurs de Killing, alors leur crochet de Lie (ou commutateur) en est également un.
\end{rmk}
\begin{rmk}
    En terme des vecteurs $X$, $Y$ et $Z$, l'algèbre s'écrit
    \begin{equation}
        [X,Y]  = Z ; \quad [Y,Z]  = X ; \quad [Z,X]  = Y
    \end{equation}
    soit pour $\xi_A = (X,Y,Z)$, on a $[\xi_A,\xi_B] = \epsilon_{ABC} \xi_C$.
\end{rmk}
\begin{theoremframe}
    \begin{defi}
        Une métrique est dite à \emph{symétrie sphérique} si et seulement si elle admet 3 vecteurs de Killing satisfaisant à (\ref{eq:algèbre de so(3)}), l'algèbre $\mathfrak{so}(3)$.
    \end{defi}
\end{theoremframe}
...
\begin{theoremframe}
    \begin{prop}
        ...
        \end{equation}
        où $\td \Omega^2 = \td \theta^2 + \sin^2\theta \, \td \varphi^2$, et $\alpha,\beta,\gamma,\delta$ sont des fonctions arbitraires en $t,r$ mais pas en $\theta$ ou $\varphi$.
    \end{prop}
\end{theoremframe}
\begin{proof}
    Cette proposition sera accepté sans démonstration dans ce cours. Un lecteur intéressé pourra consulter le Carroll p. 198.
    Notons que cette forme est une conséquence directe du fait que $\td r, \td t$ et $\td\Omega^2$ sont les seules expressions différentielles invariantes sous SO(3), et que les fonctions arbitraires ne peuvent pas dépendre de $\theta$ ou $\varphi$ pour la même raison.
\end{proof}
Pour étudier le comportement de cette métrique, intéressons-nous à quelques cas particuliers :
\begin{enumerate}
    \item À $(t,r) = (t*,r*)$ fixés :
    \begin{equation}
        \td s^2 (t^*,r^*,\theta,\varphi ) = \beta (t^*,r^*) \, \td \Omega^2
    \end{equation}
    qui est une sphère à rayon $\beta (t^*,r^*)$. Notons que si $\beta$ dépendait des variables angulaires, nous aurions une sphère imparfaite \emph{cabossée}.
    \item À $(\theta,\varphi) = (\theta*,\varphi*)$ fixés :
    \begin{equation}
...
\end{enumerate}
On peut encore simplifier la métrique en utilisant ses invariances résiduelles. Définissons une nouvelle coordonnée radiale $\tilde{r}^2(t,r) \equiv \beta (t,r)$ ainsi que $\tilde{t} = t$. Si ce changement de coordonnées est inversible, on a une nouvelle métrique 
\begin{equation}
...
\begin{rmk}
    Le changement de variables n'est valable que si $\beta (t,r)>0$. Si on suppose que la métrique est asymptotiquement plate, c'est-à-dire qu'elle tend vers la métrique de Minkowski pour $r \to \infty$, alors $\beta \to r^2$ car
    \begin{equation}
        \td s_\text{Mink}^2 = -\td t^2 +\td r ^2 + r^2 \td \Omega^2
    \end{equation}
    et sous cette hypothèse, on doit donc avoir $\beta > 0 $ au moins dans la région de l'espace-temps connecté à l'infini radial. Notons néanmoins que ce système de coordonnées peut ne pas être global (i.e. défini partout), comme nous verrons plus loin. La métrique, étant un objet défini localement, ne nous donnera a priori qu'une description locale de la géométrie !
\end{rmk}
\begin{rmk}
    On peut également questionner l'inversibilité de ce changement de variables. Par exemple, si $\beta (t,r) = \beta (t)$, la transformation n'est pas inversible. Cette possibilité est néanmoins exclue par l'hypothèse que $\beta (t,r) \to r^2$ lorsque $r\to \infty$. Alternativement, si c'était le cas, nous pourrions aussi simplement renommer $t \longleftrightarrow r$ sans problème\footnote{Voir Blau p. 483 ou Henneaux p. 98 pour détails.}.
\end{rmk}
...
\begin{align}
    ...
    R - 2R = 0 \iff R &= 0
\end{align}
et par les équations d'Einstein, on en déduit donc
\begin{align}
    R_{\mu\nu} = 0
\end{align}
...
\begin{itemize}
    ...
    \begin{equation}
        \Gamma\indices{^\mu_{\!\alpha\beta}} = \frac{1}{2} g^{\mu \rho} (\pd_\alpha g_{\beta \rho} + \pd_\beta g_{\alpha\rho} - \pd_\rho g_{\alpha \beta} )
    \end{equation}
    avec $\td s^2 = g_{\mu\nu} \, \td x^\mu \, \td x^\nu$ donné par \ref{def: Schwarzschild avant Birkhoff}.
    \item Via l'équation des géodésiques :
    \begin{equation}
        \frac{\td^2 x^\rho}{\td \lambda^2} + \Gamma\indices{^\rho_{\!\alpha\beta}} \frac{\td x^\alpha}{\td \lambda} \frac{\td x^\beta}{\td \lambda} =0
    \end{equation}
\end{itemize}
...
\end{equation}
Dénotons $\dfrac{\td u}{\td x^0} = \dot{u}$ et $\dfrac{\td u}{\td x^1} = u'$. L'équation du mouvement (\ref{eq:mvmt schwarzschild}) pour $\rho = 0$ s'écrit
\begin{align}
    \frac{\td}{\td \lambda} \left(- e^\nu \frac{\td x^0}{\td \lambda} \right) - \frac{\pd \mathcal{L}}{\pd x^0} = 0 \implies \frac{\td e^\nu}{\td \lambda} \frac{\td x^0}{\td \lambda} + e^\nu \frac{\td^2 x^0}{\td \lambda^2}+ \frac{\pd \mathcal{L}}{\pd x^0} = 0
\end{align}
Or, $\dfrac{\td }{\td \lambda} e^\nu = e^\nu \lt \dot{\nu} \dfrac{\td x^0}{\td \lambda}+ \nu' \dfrac{\td x^1}{\td \lambda}\rt$ et cette équation du mouvement se développe donc en :
\begin{align}
...
\intertext{Et en multipliant cette expression par $-e^{-\nu}$,}
...
\begin{equation}
    \frac{\td^2 x^0}{\td \lambda^2} + \Gamma\indices{^0_{\!\alpha\beta}} \frac{\td x^\alpha}{\td \lambda} \frac{\td x^\beta}{\td \lambda} =0
\end{equation}
On déduit l'expression des symboles de Christoffel
\begin{align}
    \begin{dcases}
        \Gamma\indices{^0_{00}} = \frac{\dot{\nu}}{2}\\
        \Gamma\indices{^0_{11}} = \frac{\dot{\mu}}{2} e^{\mu-\nu} \\
        \Gamma\indices{^0_{01}} = \frac{\nu'}{2}
    \end{dcases}
\end{align}
En procédant similairement\footnote{Voir Carroll p. 202 pour détails.} pour $\rho = i$, on identifie tous les autres symboles de Christoffel non-nuls :
\begin{align}
    \begin{dcases}
    ...
    \end{dcases}
\end{align}
Une fois que les symboles de Christoffel sont déterminés, il faut calculer le tenseur de Riemann selon sa définition :
\begin{equation}
    R\indices{^{\mu}_{\!\alpha \beta \gamma}} =  \partial_{\beta}\Gamma\indices{^{\mu}_{\!\gamma \alpha}} + \Gamma\indices{^{\sigma}_{\!\gamma \alpha}} \Gamma\indices{^{\mu}_{\!\beta \sigma}} - \partial_{\gamma}\,\Gamma\indices{^{\mu}_{\!\beta \alpha}} - \Gamma\indices{^{\sigma}_{\!\beta \alpha}}\,\Gamma\indices{^{\mu}_{\!\gamma \sigma}}
\end{equation}
De nouveau, calculons-en un :
\begin{align}
    R\indices{^0_{101}} &= \partial_{0}\Gamma\indices{^{0}_{11}} + \Gamma\indices{^{0}_{0 \rho}} \, \Gamma\indices{^{\rho}_{11}} - \partial_{1}\Gamma\indices{^{0}_{01}} - \Gamma\indices{^{0}_{1\rho}}\, \Gamma\indices{^{\rho}_{01}}\\
    & = \pd_0 \lt \frac{\dot{\mu}}{2} e^{\mu-\nu}\rt - \pd_1 \lt \frac{\nu'}{2} \rt + \Gamma\indices{^{0}_{00}} \, \Gamma\indices{^{0}_{11}} + \Gamma\indices{^{0}_{01}} \, \Gamma\indices{^{1}_{11}} - \Gamma\indices{^{0}_{10}} \, \Gamma\indices{^{0}_{01}} - \Gamma\indices{^{0}_{11}} \, \Gamma\indices{^{1}_{01}}\\
...
\end{align}
Les autres composantes se calculent de la même manière :
\begin{align}
...
\begin{align}
    R_{00} = R\indices{^\alpha_{0\alpha 0}} =  R\indices{^1_{01 0}}+ R\indices{^2_{02 0}}+ R\indices{^3_{03 0}}
\end{align}
Or, nous n'avons que $ R\indices{^0_{\alpha 0 \alpha}}$ pour $\mu = 0,1,2,3$. Nous devons donc convertir les composantes connues vers ceux qui interviennent dans le tenseur de Ricci :
\begin{align}
     R\indices{^1_{01 0}} = g^{1\rho} R_{\rho 010} &= g^{11} R_{1010}
     \intertext{comme la métrique est diagonale.}
     & = g^{11}R_{0101}\\
     \intertext{en appliquant les propriétés d'antisymétrie deux fois.}
...
\begin{align}
    ...
    \begin{dcases}
        R_{01} = \frac{\dot{\mu}}{r} \\
        R_{11} = e^{\mu-\nu} \, \lt \blue{\frac{\dot{\mu}}{4} ( \dot{\mu} - \dot{\nu} ) +\frac{\ddot{\mu}}{2}} \rt \purple{- \frac{\nu''}{2} + \frac{\nu' \mu'}{4} - \frac{\nu'^2}{4}} + \frac{\mu'}{r} \\
...
\end{equation}
et en particulier, $\ddot{\mu} = 0$. Remarquons que par les équations d'Einstein, la combinaison suivante s'annule :
\begin{equation}
\label{eq:combinaison: eq d'Einstein pour Schwarzschild}
    R_{00} + e^{\nu-\mu} R_{11} = 0
\end{equation}
...
\begin{theoremframe}
    \begin{defi}
        ...
        \begin{enumerate}[label= \roman*.]
            \item indépendante du temps,
            \item invariante sous inversion du temps $t \to -t$.
        \end{enumerate}
    \end{defi}
\end{theoremframe}
...
\begin{rmk}
    Une solution extérieure à symétrie sphérique étant nécessairement indépendante du temps exclut l'existence de rayonnement gravitationnel monopolaire : une étoile qui pulse sphériquement ne donne pas lieu à des ondes gravitationnelles autour d'elle dans le vide : la géométrie reste indépendante du temps et ne change pas
\end{rmk}
Nous avons pour le moment utilisé $R_{00}$, $R_{01}$ et $R_{11}$. Nous pouvons donc a priori encore contraindre la solution en utilisant $R_{22}$ (mais $R_{33}$ ne fournira pas d'information supplémentaire). Suivant notre choix de coordonnées, $\nu + \mu = 0$ et donc
\begin{align}
...
\begin{equation}
    e^{-\mu} = e^\nu = 1 - \frac{2m}{r}
\end{equation}
ce qui nous emmène à la forme finale de la \emph{métrique de Schwarzschild} :
\begin{align}
...
\end{exerc}
La condition \emph{asymptotiquement plate} garantissait que le changement de coordonnées $\tilde{r}^2 = Y^2(t,r)$ était inversible. On peut montrer que ce n'est pas nécessaire pour se ramener à la métrique de Schwarzschild. Les différents cas de figure sont classifiés par le signe de $\pd_\mu Y \pd^\mu Y$ (pour $Y = \text{cste}$ étant de genre temps, nul ou espace)\footnote{Voir MTW, EH pour détails.}.
Par exemple, si $Y(t,r) = t$, on se ramène à la solution \emph{intérieure à l'horizon} de Schwarzschild, à laquelle on s'intéressera au prochain chapitre. On peut également vérifier explicitement que $Y(t,r) = R = \text{cste}$ ne satisfait pas aux équations d'Einstein.
\begin{rmk}
...
\end{rmk}
\begin{rmk}
    Notons que nous n'avons utilisé qu'une combinaison de $R_{00}$ et $R_{11}$. Il s'avère qu'avec les équations utilisées, nous aurons nécessairement $R_{00} = 0 = R_{11}$. Autrement dit, la combinaison (\ref{eq:combinaison: eq d'Einstein pour Schwarzschild}) contenait toute l'information indépendante de ces deux composantes.
\end{rmk}
...
\begin{exmp}
    ...
    \begin{equation}
        \td s^2 = \frac{\td \rho^2}{(\rho - 2m)^4} + \frac{\td \Omega^2}{(\rho -2m)^2}
    \end{equation}
    qui diverge à présent pour $\rho=2m$. Bien-sûr, ce comportement est dû au fait que les coordonnées polaires pour l'espace plat euclidien ne sont pas bien définies en $r=0$. Le point physique en $r=0$ n'est pas différent des autres, c'est seulement que le système de coordonnées n'est pas approprié pour y décrire la variété\footnote{En se rappelant des notions de cartes locales, le point $r=0$ n'était en fait pas inclut dans celle-ci : il nous faut une autre carte locale pour décrire ce point (ce qui ne devrait pas nous étonner, comme nous avions précédemment montré que la sphère n'admet pas de carte globale).}. En effet, rien de spécial s'y passe lorsqu'on considère les coordonnées cartésiennes usuelles. 
\end{exmp}
...
Concentrons-nous sur les singularités potentielles de la métrique de Schwarzschild (\ref{métrique de Schwarzschild}). On souhaite construire des quantités scalaires non-triviales. Comme (\ref{métrique de Schwarzschild}) est une solution des équations d'Einstein, la courbure scalaire et $R_{\mu\nu} R^{\mu\nu}$ s'annulent identiquement. Le tenseur de Ricci n'étant pas utile, nous devons recourir au tenseur de Riemann. Le scalaire le plus simple de cette forme est le \emph{scalaire de Kretschmann} défini comme contraction complète du tenseur de Riemann avec lui-même :
\begin{equation}
    K = R_{\lambda \mu \alpha \beta} R^{\lambda\mu\alpha\beta}
\end{equation}
Cette contraction contient a priori $4^4 = 3.600$ termes. Pour la calculer, il faudra donc utiliser au plus possibles les propriétés de symétrie et les relations (\ref{Riemann pour Schwarzschild}) du tenseur de Riemann pour la métrique de Schwarzschild en décomposant incrémentalement les contractions.
\begin{align}
    ...
    &= \cancel{R_{00\alpha\beta} R^{00\alpha\beta}} + R_{0i\alpha\beta} R^{0i\alpha\beta} + R_{i0\alpha\beta} R^{i0\alpha\beta} + R_{ij\alpha\beta} R^{ij\alpha\beta}
\end{align}
...
\begin{align}
    ...
    =& \,2 \lt \cancel{R_{0 i 00} R^{0 i00}} +  R_{0 i 0j} R^{0 i0j} + R_{0 ij0} R^{0 ij0} + R_{0 ijk} R^{0 ijk}\rt \\
    &+ \cancel{R_{ij00} R^{ij00}} + R_{ij0k} R^{ij0k} + R_{ijk0} R^{ijk0} + R_{ijkl} R^{ijkl} 
\end{align}
...
\begin{align}
    ...
    =& \, 4  R\indices{^0_{i 0j}} R\indices{_0^{ i0j}} + 4 R\indices{^0_{ ijk}} R\indices{_0^{ ijk}} + R\indices{^i_{jkl}} R\indices{_i^{jkl}}
\end{align}
...
\begin{align}
    R\indices{_0^{101}} = g_{0\alpha}\, g^{1\beta}\, g_{0\gamma} \, g^{1 \delta} \, R\indices{^\alpha_{\beta \gamma \delta}} & = \cancel{g_{00}}\,  g^{11} \, \cancel{g^{00}}\,  g^{11}\,  R\indices{^0_{101}} \\
...
\end{equation}
On en conclut qu'en $r = 0$, la métrique est bel et bien \emph{singulière}. Rappelons que le tenseur de Riemann était responsable de l'accélération relative entre géodésiques, en d'autres termes, responsable de \emph{forces de marée}. L'équation (\ref{Kretschmann}) indique qu'en $r=0$, l'amplitude des forces de marées devient infinie ! Cela pose problème, surtout si la singularité peut être atteinte en un temps fini (ce que nous étudierons plus tard).
...
\\
Ceci amène la question suivante : la métrique de Schwarzschild est-elle utile (bien qu'elle présente des pathologies en $r=2m$) ? Premièrement, évaluons $r_\text{Schw}=2m $ (appelé \emph{rayon de Schwarzschild}) pour certains corps célestes familiers.
\begin{itemize}
    \item Soleil : $r_\text{Schw} \sim 3$km, à comparer avec son rayon $R_\odot \sim 700.000$km.
    \item Terre : $r_\text{Schw} \sim 1$cm, à comparer avec son rayon $R_\oplus \sim 6370$km.
\end{itemize}
Donc, ici et pour la plupart des corps célestes, le rayon de Schwarzschild est bien inférieure au rayon de l'astre. Or, comme la solution de Schwarzschild n'est valable qu'à l'extérieur de l'astre, la métrique de Schwarzschild est parfaitement valable pour décrire la géométrie à l'extérieur d'astres qui ont $R \gg 2m$. Notons également que les coordonnées $t$ et $r$ de la métrique de Schwarzschild correspondent aux temps et rayons usuels observés par un observateur lointain ($r \sim \infty$), pratique pour les situations réalistes.
...
\begin{equation}
    ...
    \frac{\td^2 x^\alpha}{\td \lambda^2} + \Gamma\indices{^\alpha_{\!\beta\gamma}} \frac{\td x^\beta}{\td \lambda} \frac{\td x^\gamma}{\td \lambda} = 0
\end{equation}
...
\end{equation}
On peut supposer que le mouvement du corps se fait dans le plan $\theta = \frac{\pi}{2}$, résultant de la conservation de la direction du vecteur moment angulaire, qui est une constante du mouvement pour un potentiel central. Le mouvement s'effectue donc dans un plan, qu'on fixe à $\frac{\pi}{2}$. Le système possède deux (autres) intégrales premières :
\begin{enumerate}
...
\end{enumerate}
Comme en coordonnées polaires, la vitesse s'écrit $\vect{v} = \dot{r} \vect{u}_r + r \dot{\varphi} \vect{u}_\varphi$, on obtient
\begin{equation}
    \frac{1}{2} (\dot{r}^2+r^2\dot{\varphi}^2) - \frac{GM}{r} = E
\end{equation}
Par la première constante du mouvement, $r^2 \dot{\varphi}^2 = a^2/r^2$ et on obtient finalement
\begin{equation}
    \label{eq:kepler newton}
    \frac{1}{2} \dot{r}^2 = E - V_\text{eff}(r), \quad V_\text{eff}(r) \equiv \frac{a^2}{2r^2} - \frac{GM}{r}
\end{equation}
le premier terme étant associé au potentiel centrifuge et le deuxième au potentiel gravitationnel. Les trajectoires possibles d'une particule sont obtenues en comparant l'énergie $E$ à $V_\text{eff}(r)$, dont la forme dépend de $a$. En effet, si  $E < V_\text{eff}(r)$ alors $\dot{r}^2 < 0$ : la trajectoire est impossible.\\
\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Chapitres/7. La solution de Schwarzschild/Images/orbites newtoniens.pdf}
    \caption{Représentation du potentiel pour deux valeurs de $a$ (et $GM=1$). Différentes valeurs d'énergie des particules vont déterminer le type de mouvement suivi. Deux énergies y sont illustrés, pour lesquelles la particule suit soit une ellipse $(E_1$) ou une hyperbole ($E_2$). Pour une énergie donnée, sont permis uniquement les rayons tels que $V<E$. Lorsque la trajectoire arrive en $V=E$, elle est réfléchie.}
    \label{fig:potentiel keplerien classique}
\end{figure}
Que devient cette analyse en relativité générale ? L'ensemble d'équations différentielles (\ref{eq: géodésiques pour la métrique de Schw}) sont des EDOs couplées du second ordre. Heureusement, dans le cas de Schwarzschild (et aussi pour d'autres métriques avec un certain degré de symétrie), on peut exploiter les symétries du problème pour en tirer des \emph{constantes du mouvement}. L'équation des géodésiques dérive d'un principe variationnel pour
\begin{equation}
...
Si la métrique est indépendante d'une coordonnée ($t$, ci-dessus), on obtient immédiatement une quantité conservée le long des géodésiques. Ceci se généralise à tout vecteur de Killing de la métrique (pas nécessairement de la forme simple $\xi = \pd_t$).
\begin{theoremframe}
    \begin{propri}
...
\end{proof}
La métrique de Schwarzschild est à symétrie sphérique. Elle possède donc 3 vecteurs de Killing formant $\mathfrak{so}(3)$. Ceux-ci fournissent 3 constantes du mouvement supplémentaires : deux combinaisons linéaires indépendantes de ces constantes du mouvement impliquent que le mouvement s'effectue dans un plan : $\dot{\theta} = 0$. Vous aurez l'honneur de prouver ce résultats en séance d'exercices. On choisira un système de coordonnées tel que $\theta  = \frac{\pi}{2}$. Ceci correspond en mécanique newtonienne à la conservation de la direction du moment angulaire.\\
...
\begin{equation}
    \frac{\pd \mathcal{L}}{\pd \dot{\varphi}} = r^2 \dot{\varphi} = a
\end{equation}
...
\end{equation}
C'est un effet très petit, mais néanmoins observable si cumulatif (voir plus loin, avancée du périhélie).
\begin{rmk}
    Dans l'équation (\ref{géodésiques de Schwarzschild}), chacun des termes est sans dimensions (évident via le terme $\frac{1}{2}$). En particulier, $[\dot{r}] = 1$ car
    \begin{equation*}
...
\end{equation}
Les orbites circulaires sont données par la condition
\begin{equation}
...
\begin{enumerate}
    ...
    \item Si $a^2>12m^2$, il y aura deux racine réelles et donc deux orbites circulaires possibles, l'une stable et l'autre instable (déterminé en calculant le signe de $V''_\text{eff} (r)$). On trouve que $r_\text{instable}<6m$ et que $r_\text{stable}>6m$.
\end{enumerate}
On remarque donc que $r=6m$ est le rayon minimal pour avoir une orbite circulaire stable (également appelé ISCO : \emph{innermost stable circular orbit}). En effet, $V''_\text{eff} (r=6m) =0$ et $V'''_\text{eff} (r=6m)>0$. En théorie newtonienne, ces orbites existaient pour tout $a$.\\
\\
...
Ainsi, 
\begin{enumerate}
    \item Pour $a^2<16m^2$, il n'y a pas de zéros : $\dot{r}$ ne s'annule pas et la trajectoire ne changera jamais de sens (radialement).
...
\begin{rmk}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{Chapitres/7. La solution de Schwarzschild/Images/orbites non-bornées.png}
        \caption{Illustration d'une orbite non-bornée.}
        \label{fig:schwarzschild orbites non-bornées}
    \end{figure}
\end{rmk}
\begin{rmk}
    \label{rmk: orbites oscillatoires}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{Chapitres/7. La solution de Schwarzschild/Images/orbites bornées oscillantes.png}
        \caption{Illustration d'une orbite bornée oscillante.}
        \label{fig:schwarzschild orbites bornées oscillante}
    \end{figure}
\end{rmk}
\begin{rmk}
...
\end{rmk}
L'analyse présente ne dit pas ce qu'il se passe pour $r \leq 2m$ car la métrique y est mal définie. En particulier, elle ne nous dit pas si cette région peut être atteinte, c'est-à-dire si elle se trouve à l'extérieur de l'astre gravitant.
\begin{figure}[H]
        \centering
        \includegraphics[width=0.9\linewidth]{Chapitres/7. La solution de Schwarzschild/Images/orbites relativistes.png}
        \caption{Illustration d'une orbite bornée oscillante.}
        \label{fig:schwarzschild orbites}
    \end{figure}
\subsection{Approche par orbites circulaires}
Pour s'approcher d'un astre gravitant en passant d'une orbite circulaire à l'autre dans le formalisme newtonnien, il suffisait de diminuer son moment angulaire (en boostant à contre-sens le long de l'orbite par exemple). La même chose se produit en relativité générale si l'on part de $r^c_1$ et que l'on s'approche jusqu'à $r^c_4 = 6m \equiv$ ISCO. À cet endroit, pour passer vers une orbite circulaire de rayon plus petit, il faut \emph{augmenter} $a$ : on passe à $r^c_5$. Comme il s'agit d'une orbite instable, si on diminue $r$ en conservant $a$, on \emph{chute}. On peut alors, en augmentant $a$, passer par des orbites de rayons de plus en plus petits jusqu'à approcher $r=3m$, que l'on ne pourra jamais atteindre avec une orbite circulaire car cela nécessiterait $a = \infty$. On peut explorer $r<3m$ en suivant une géodésique, mais celle-ci ne pourra pas être circulaire : on plonge.
...
\end{equation}
Pour un paramètre affine $\lambda$. Attention que $\lambda \neq a\tau +b$ dans ce cas, comme le temps propre s'annule identiquement et n'est donc pas un paramétrage adéquat. Les autres constantes du mouvement restent inchangées, c'est-à-dire :
\begin{align}
    \begin{dcases}
        \dot{\theta} = 0 \\
        e^\nu \dot{t} = b \\
...
\begin{equation}
    \frac{1}{2} \lt \lt \frac{\td r}{\td \varphi}\rt^2 \frac{c^2a^2}{r^4} + \frac{c^2a^2}{r^2} \rt - \frac{GM}{r} = \text{cste} 
\end{equation}
On pose $u = \frac{1}{r}$ et donc $\frac{\td u}{\td \varphi} = - \frac{1}{r^2} \frac{\td r}{\td \varphi}$ :
\begin{equation}
    \frac{1}{2} \lt \lt \frac{\td u}{\td \varphi}\rt^2 c^2 a^2 + c^2a^2 u^2 \rt - GM u = \text{cste} 
\end{equation}
...
\end{equation}
On cherche $r(\varphi)$ :\footnote{Où $\dot{x}$ correspond à une dérivée par rapport au paramètre affine de la géodésique.}
\begin{equation}
...
\end{align}
Comme on peut prendre $a\neq 0$ (via l'hypothèse du régime oscillatoire donné par la remarque \ref{rmk: orbites oscillatoires}), 
\begin{align}
     \lt \frac{\td u}{\td \varphi} \rt^2  + u^2 &= \frac{b^2 -1}{a^2} + \frac{2m}{a^2}u +2mu^3
\end{align}
En différenciant par rapport à $\varphi$, on obtient finalement (avec l'hypothèse que $\frac{\td u}{\td \varphi} \neq 0$, qui représente un mouvement circulaire) :
\begin{align}
...
\end{exerc}
On s'intéresse au cas $e<1$, i.e. aux orbites elliptiques. On écrit la solution générale comme
\begin{equation}
    u = u_0 +u_1, \quad u_1 \ll u_0
\end{equation}
En injectant cette forme dans (\ref{équation de Binet RG}), on obtient au premier ordre en $u_1$
\begin{align}
    \frac{\td^2 u_0}{\td \varphi^2} +\frac{\td^2 u_1}{\td \varphi^2} + u_0 + u_1 = \frac{m}{a^2} + 3m \lt u_0^2 + 2u_0u_1 +u_1^2 \rt\\
    \implies \frac{\td^2 u_1}{\td \varphi^2} + u_1 = + 3m \lt u_0^2 +2u_0u_1 \rt + \mathcal{O}(u_1^2)
\end{align}
Où on a utilisé le fait que $u_0$ est solution de l'équation de Binet. On peut effectuer une autre simplification en réécrivant l'équation sous la forme suivante :
\begin{equation}
...
\end{equation}
Par l'identité trigonométrique $\cos^2\varphi = \frac{1 + \cos 2\varphi}{2}$, on peut développer le carré et obtenir l'expression finale
\begin{align}
...
\end{align}
On s'intéresse à la correction $u_1$ qui sera responsable de la non-périodicité de l'orbite. La solution générale de (\ref{Périhélie équation perturbative}) est donnée par la combinaison d'une solution générale de l'équation homogène (SGEH) et d'une solution particulière de la solution périodique non-homogène (SPENH). Une telle solution particulière est donnée par
\begin{equation}
...
\begin{align}
    ...
    \end{dcases}
\end{align}
Cette solution doit être ajoutée à la solution non-perturbée $u_0$. Les termes constants et $2\pi$-périodiques de (\ref{Périhélie solution perturbative}) (premier et second termes) sont négligeables par rapport au termes constants et $2\pi$-périodiques de $u_0$ : comme $u_0 \propto m/a^2$ et $u_1 \propto m^3/a^4$ on trouve
\begin{equation}
...
\end{align}
\begin{rmk}
    Notons que la trajectoire perturbée n'est plus à strictement parler une ellipse à cause du terme en $\cos 2 \varphi$, même si à l'exception du terme $\varphi \sin\varphi$ elle reste périodique. Les paramètres de cette \emph{ellipse} ont également des corrections de l'ordre de $10^{-7}$.
\end{rmk}
On se concentrera de garder le dernier terme qui, bien qu'également petit est \emph{non-périodique} : son effet sera cumulatif !
