...
\begin{align}
    \frac{\td f}{\td f} = 1 \longrightarrow \frac{\delta f(x)}{\delta f(y)} = \delta (x-y)
\end{align}
Il est intéressant de dériver ces équations à partir du principe de moindre action, comme il est souvent plus simple de faire varier l'action que de se souvenir des équations d'Euler-Lagrange (notamment pour des actions plus compliquées).
\begin{proof}
    ...
    \begin{equation}
        ...
        \right.
    \end{equation}
    La variation de la densité lagrangienne (qu'on appelera par abus de language également Lagrangien) s'écrit sous ces variations comme 
    \begin{align}
    ...
    \end{equation}
    où $n_{\mu}$ est le champ de vecteurs normal sortant du bord du domaine. On remarque que ce terme s'annule effectivement si les variations aux bord du domaine s'annulent (ce qui peut être imposé en demandant que les champs s'annulent toujours au bord). Finalement, on obtient bien que si
    \begin{align}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item L'élément de mesure $\td ^4x' = |\det \frac{\partial x'}{\partial x}|\td ^4x$ où $|\det \frac{\partial x'}{\partial x}|$ est le déterminant de la jacobienne de la transformation. Or, pour une transformation de Poincaré, $x' = \Lambda x + a$ où $\Lambda$ vérifie $\det \Lambda = \pm 1$. 
...
\begin{align}
    ...
    & =\td ^4x \sqrt{-g}
\end{align}
Ce nouveau élément de mesure est donc indépendant du choix de coordonnées. Comme dans un RLI, il se ramène à l'unité, par le principe d'équivalence, il correspond bien à la généralisation covariante de l'élément de mesure. Par conséquent l'action d'Einstein-Hilbert définit bien une quantité invariante sous changement général de coordonnées.
\begin{rmk}
...
\end{theoremframe}
\begin{proof}
    Par définition, le tenseur de Riemann est donné l'expression
    \begin{equation}
...
\begin{proof}
    ...
    \end{align}
    En remplaçant ce résultat dans l'équation \ref{eq:variation de R}, on obtient
    \begin{align}
...
\begin{equation}
    \int_{V} \td^4 x \, \sqrt{-g} \, \nabla_{\alpha}V^{\alpha} = \int_{V} \td^4 x \, \partial_{\alpha}( \sqrt{-g}\,V^{\alpha}) = \int_{\partial V} \td^3 x \, n_{\alpha}\sqrt{-g}\,V^{\alpha} = 0
\end{equation}
...
\begin{equation}
   G^{\mu \nu} + \Lambda g^{\mu \nu} = 0
\end{equation}
et on retrouve bien l'équation d'Einstein à constante cosmologique non-nulle.
...
\end{equation}
où $F^{\mu \nu}$ dépend implicitement de la métrique à travers ses contractions. Les équations du mouvement par rapport au champ dynamique source $A_\mu$ donnent toujours les équations de Maxwell :
\begin{equation}
...
\cutebreak
\begin{rmk}
    On considère les unités tel que $\hbar = 1$, $c = 1$. Comme la dimension de l'action $[S]$ est $\hbar$, dans ce système d'unités, $\hbar = 1$, l'action est sans unités. 
    \begin{align}
...
Nous allons montrer qu'un des avantages du principe variationnel pour les équations d'Einstein est qu'il permet d'implémenter les symétries de la théorie et ses conséquences. En particulier, nous allons analyser les conséquences de la covariance de $S_{\text{EH}}$ (et dans certain cas, de son invariance). Nous avons vu que pour une variation arbitraire de la métrique, l'action variait comme 
\begin{equation}
    \delta S = -\int \td^4 x \sqrt{-g}\,G^{\mu\nu}\delta g_{\mu \nu}
    \label{eq:variation de l'action d'Einstein-Hilbert}
\end{equation}
Nous allons particulariser cette équation pour des variations sous difféomorphisme (transformation générale des coordonnées) :
\begin{equation}
    \begin{dcases}
...
Sous une transformation générale des coordonnées $x^{\alpha } \to x'^{\alpha }$, les transformations tensorielles correspondantes sur les champs sont par exemple :
\begin{enumerate}
