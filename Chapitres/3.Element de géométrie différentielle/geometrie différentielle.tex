...
La relativité générale repose sur le PE qui stipule que, dans des régions suffisamment restreintes de l'espace-temps, un observateur ne peut distinguer un champ de gravitation d'une accélération constante. Il lui est donc impossible de détecter l'existence d'un champ de gravitation par le biais d'expériences locales : les lois de la physique doivent se réduire dans ces régions à celles de la relativité restreinte, dont nous avons revu quelques éléments-clefs. Une conséquence directe du PE est le décalage vers le rouge (redshift) de photons s'élevant dans un champ de gravitation, ce qui nous avait suggéré que ceux-ci se déplaçaient dans un espace-temps courbe.

Nous sommes donc amené\!·\!es à décrire l'espace-temps par une structure mathématique qui ressemble localement à $\R^{1,3}$, mais qui peut posséder une \emph{courbure} non-triviale sur des régions étendues\footnote{De la même manière que le principe de relativité, et l'invariance de la vitesse de la lumière dans le vide pour tout référentiel inertiel, ont amené à unifier l'espace et le temps dans une structure commune via les transformations de Lorentz, à l'invariance de l'intervalle et à l'espace-temps de Minkowski.}.
...
Nous allons introduire en premier lieu la notion de \emph{tenseur} dans l'espace-temps plat (section \ref{sec:tenseurs plats}), pour ensuite s'intéresser aux \emph{variétés} (section \ref{sec:variétés}) qui nous permettrons de généraliser les définitions de tenseurs à des espace-temps courbes (section \ref{sec:tenseurs variétés}). Finalement, nous verrons à la section \ref{sec:formes} une classe de tenseurs particulièrement utile, appelées \emph{formes différentielles}.

Une attention particulière sera portée à l'ordre des indices des matrices $\Lambda\indices{^\mu_\nu}$, qui n'est inclus que pour une raison de rigeur. En pratique, ce détail est sans importance et peut être omis.
\section{Tenseurs dans un espace-temps plat}
\label{sec:tenseurs plats}
...
\begin{theoremframe}
    \begin{notat}
        Soit $P\in \mathcal{M}$. L'espace tangent à $\mathcal{M}$ au point $P$ est noté $T_P\mathcal{M}$. C'est un espace vectoriel réel de dimension 4. Les éléments de $T_P\mathcal{M}$ sont appelés \textit{vecteurs} de $\M$ au point $P$. 
    \end{notat}
\end{theoremframe}
Pour le moment, nous nous abstenons de définir rigoureusement ce qu'est exactement un \emph{vecteur}. Pour $\R^{1,3}$, on peut en effet se fier à la notion usuelle, avec deux exceptions :
\begin{enumerate}[label=(\roman*).]
    \item Un vecteur n'est plus défini sur l'espace $\M$ lui-même, mais sur son \emph{espace tangent}. Le vecteur \textit{ne vit pas} dans le même espace que les points de $\M$.
    \item Le vecteur est défini \emph{par rapport} à un point $P\in \M$ : c'est donc une notion locale. On pourra donc par exemple additionner deux vecteurs issus du même espace tangent $T_P\M$, mais pas s'ils appartiennent à des espaces tangents distincts.
\end{enumerate}
Lorsque $\mathcal{M} = \R^{1,3}$, il n'y a pas de problème car les ensembles $\mathcal{M}$ et $T_P\mathcal{M}$ coincident (en vertu du fait que $\R^{1,3}$ est lui-même un espace vectoriel) : on peut alors les voir comme des flèches rejoignant deux points distincts. Ce ne sera plus le cas si l'espace-temps est courbe.
\begin{theoremframe}
    \begin{defi}
        L'union disjointe de tous les espaces tangents de $\mathcal{M}$, noté $T\mathcal{M}$, est appelé le \textit{fibré tangent} de $\mathcal{M}$ :
        \begin{align}
            \nonumber
    ...
    \end{defi}
\end{theoremframe}
Comme $T_P\mathcal{M}$ est un espace vectoriel à 4 dimensions, il existe une base de vecteurs $\{\mathrm{e}_\alpha\}_\alpha$ de $T_P\mathcal{M}$ où $\alpha = \{0,1,2,3\}$. Un vecteur $V \in T_P\mathcal{M}$ peut donc être décomposé sur cette base selon $V = V^\alpha \mathrm{e}_\alpha$, où $V^\alpha$ sont les composantes du vecteur $V$. Bien que $V$ ne dépend pas du choix de la base, $V^\alpha$ dépend bien de ce choix.
\begin{theoremframe}
    \begin{defi}
        La donnée d'un vecteur $x_P \in T_P\M$ en tout point $P \in \M$ est un champ de vecteurs :
        \begin{align*}
            U : \begin{pmatrix}
...
\end{theoremframe}
\begin{rmk}
    L'utilisation du fibré tangent se justifie donc par la notion locale d'un vecteur : pour chaque vecteur, il faut spécifier le point par rapport auquel il est défini. Comme avant, ceci reste superflu pour $\R^{1,3}$ et plus généralement, on admettera l'abus de notation en omettant la première composante du champ, $U: P \to U(P)$.
\end{rmk}
L'exemple canonique de champ de vecteur est le champ tangent à une courbe :
\begin{exmp}
    Soit une courbe lisse $\gamma :\R \to \mathcal{M}: \lambda \mapsto x^\mu (\lambda)$. Le vecteur tangent à la courbe est
    \begin{equation}
...
\end{theoremframe}
\begin{proof}
    Soit un vecteur $V \in T_P\M$ et une courbe lisse $x^\mu(\lambda)$ telle que le vecteur tangent à la courbe au point $P$ est $V$. Sous transformation de Poincaré $(\Lambda,a)$, cette courbe se transforme comme $x^{\mu'} = \Lambda\indices{^{\mu'}_{\mu}} \, x^\mu + a^{\mu'}$. Alors, par définition de $V$, on trouve
    \begin{align}
        V^{\mu'} = \frac{\td x^{\mu'}}{\td \lambda} = \Lambda\indices{^{\mu'}_{\mu}} \, \frac{\td x^{\mu}}{\td \lambda} = \Lambda\indices{^{\mu'}_{\mu}}\, V^\mu
    \end{align}
\end{proof}
\begin{notat}
    Dans la suite du cours, nous expliciterons des changements de référentiels par des indices (usuellement un prime dans ce syllabus), et non sur les quantités elles-mêmes. Par exemple : le vecteur $V$ peut s'écrire $V^\alpha$ dans un référentiel et $V^{\alpha'}$ dans un autre référentiel (à la place de $V'^{\alpha}$). Cette notation permet de facilement distinguer quel indice provient de quel référentiel. Il n'y a par ailleurs aucun lien entre la notation des vecteurs de base et les indices.
\end{notat}
\begin{theoremframe}
    \begin{propri}
        Sous transformation de Poincaré, les vecteurs de base se transforment comme
        \begin{equation}
            \mathrm{e}_{\mu'} = \Lambda\indices{_{\mu'}^{\mu}}\, \mathrm{e}_{\mu}
        \end{equation}
    \end{propri}
\end{theoremframe}
\begin{proof}
    ...
    \begin{equation*}
        V = V^{\mu} \, \mathrm{e}_{\mu} = V^{\alpha'} \,\mathrm{e}_{\alpha'}
    \end{equation*}
    Ainsi,
    \begin{equation*}
        V^{\alpha'}\, \mathrm{e}_{\alpha'} = \Lambda\indices{^{\alpha'}_{\mu}} \, V^\mu \,\mathrm{e}_{\alpha'} = V^\mu \,\mathrm{e}_{\mu}
    \end{equation*}
    Comme ceci est valable pour $V^\mu$ arbitraire, on conclut que 
    \begin{equation}
        \label{eq:preuve loi base vecteur}
        \Lambda\indices{^{\alpha'}_{\mu}} \,\mathrm{e}_{\alpha'} = \mathrm{e}_{\mu}.
    \end{equation}
    En remarquant que $\Lambda$ vérifie\footnote{Via $\Lambda \eta \Lambda^T = \eta$.}
    \begin{equation}
    \nonumber\Lambda\indices{_{\alpha}^\mu}\, \eta_{\mu\nu} \,\Lambda\indices{_{\beta}^\nu} = \eta_{\alpha\beta}
    \end{equation}
    ...
    \end{equation}
    Ceci permet de conclure, en multipliant l'équation (\ref{eq:preuve loi base vecteur}) par $\Lambda\indices{_{\mu'}^{\mu}}$ que
    \begin{equation}
    \nonumber
        \mathrm{e}_{\mu'} = \Lambda\indices{_{\mu'}^{\mu}}\, \mathrm{e}_{\mu}
    \end{equation}
...
\begin{theoremframe}
    \begin{defi}
        Une forme linéaire sur un espace vectoriel réel $(\mathcal{V},\R)$ est une application linéaire $w:\mathcal{V} \to \R$. Pour $a,b\in \R$ et $X,Y \in \mathcal{V}$, on a donc
        \begin{equation}
            w(aX+bY) = aw(X)+bw(y)
        \end{equation}
    \end{defi}
...
\begin{align*}
    w(V) &= w_{\alpha} \Theta^{\alpha}(V^{\mu} \mathrm{e}_\mu)\\
    &=w_{\alpha} V^{\mu}\Theta^{\alpha} (\mathrm{e}_{\mu})\\
...
\end{theoremframe}
\begin{exmp}
    Si on considère un vecteur comme un élément du dual de l'espace cotangent (équivalent d'après la remarque \ref{rmk:tangent^2}) :
\begin{align*}
...
\begin{theoremframe}
    \begin{propri}
        ...
        \begin{equation}
        \mathrm{e}_{\alpha_1} \otimes \cdots \otimes \mathrm{e}_{\alpha_k} \otimes \Theta^{\mu _1}\otimes \cdots \otimes \Theta^{\mu _l}
        \end{equation}
    \end{propri}
\end{theoremframe}
Un tenseur de type $(k,l)$ se décompose donc sur cette base comme
\begin{equation}
    T = T\indices{^{\alpha _1 \cdots \alpha _k}_{\nu_1 \cdots \nu_l}}\mathrm{e}_{\alpha_1} \otimes \cdots \otimes \mathrm{e}_{\alpha_k} \otimes \Theta^{\nu_1}\otimes \cdots \otimes \Theta^{\nu_l}
\end{equation}
\begin{theoremframe}
    \begin{propri}
        Un tenseur de type $(k,l)$ se transforme sous transformation de Poincaré comme\footnote{Ici, $\times$ est utilisé pour la multiplication usuelle et non comme produit direct.}
        \begin{equation}
            T\indices{^{{\alpha}_1' \cdots {\alpha}_k'}_{{\nu_1'} \cdots {\nu_l'}}} = \Lambda\indices{^{\alpha_1'}_{\!\alpha _1 }} \times \cdots \times \Lambda\indices{^{\alpha_k'}_{\!\alpha_k}} \times \Lambda\indices{_{\nu_1' }^{\!\nu_1 }} \times \cdots \times \Lambda\indices{_{\nu_l'}^{\!\nu_l}} \times T\indices{^{\alpha _1 \cdots \alpha _k}_{\nu_1 \cdots \nu_l}}.
        \end{equation}
...
Revenons à la métrique de Minkowski. Celle-ci peut-être vue comme un tenseur $(0,2)$ symétrique.
{
\renewcommand\indexmarker{\cdot}
\begin{equation*}
...
\begin{theoremframe}
    \begin{defi}
        ...
        \end{pmatrix}.
        \end{align}
        En particulier, $\eta$ est symétrique et bilinéaire.
    \end{defi}   
\end{theoremframe}
\begin{rmk}
...
\begin{align}
    \begin{dcases}
        ...
        \vect{\nabla} \cdot \vect{B} = 0\\
        \vect{\nabla} \times E + \partial_t \vect{B} = \vect{0}
    \end{dcases}
\end{align}
...
\begin{proof}
    ...
    \end{equation}
    Ceci montre bien que dans l'exemple des lois de Maxwell, son expression reste la même dans tout référentiel inertiel, en vertu de se transformer sous une loi de transformation covariante (sous Poincaré).
    }
\end{proof}
\section{Variétés différentielles}
\label{sec:variétés}
Nous allons à présent considérer un espace-temps $\mathcal{M}$ avec courbure. Introduisons d'abord quelques notions périphériques, pour ensuite définir les \emph{variétés différentielles}. Nous verrons au chapitre suivant que ce formalisme donnera effectivement lieu à une notion de courbure.
\begin{theoremframe}
    \begin{defi}
        ...
        \end{enumerate}
        Le couple $(X,\mathcal{T}_X)$ est un \textit{espace topologique}. Les éléments de $U\in \mathcal{T}_X$ sont les ouverts de l'espace topologique.\footnote{Voir p.ex. Nakahara p.81. pour plus de détails.}
    \end{defi}
\end{theoremframe}
Sur base de cette définition, il est possible de reconstruire les notions d'analyse comme la continuité, comme approche alternative à l'approche séquentielle (via des suites). Nous n'allons pas aller en détail dans cette approche et directement passer au sujet de la géométrie différentielle.
\begin{theoremframe}
    \begin{defi}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{Chapitres/3.Element de géométrie différentielle/Images/transformation actives.jpg}
        \caption{Représentation active d'une transformation des coordonnées.}
        \label{fig:transf actives}
    \end{figure}
\item \textbf{Transformation passive des coordonnées}\\
Soient deux cartes locales de $P$, notées $(U,\varphi)$ et $(V,\psi)$ et notons $\varphi(P) = x^\mu(P)$ et $\psi(P) = y^\mu(P)$. Remarquons que ces deux coordonnées ne coïncident pas nécessairement. Ils sont liés par la fonction de transition $f = \varphi \circ \psi^{-1}, \, y^\mu \mapsto x^\mu$, qui est lisse (c'est une fonction $\R^n \to \R^n$). Cette représentation est nommée \emph{passive}, car le point $P$ reste le même : la différence vient des deux cartes distinctes utilisées.
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Chapitres/3.Element de géométrie différentielle/Images/transformation pastive.jpg}
    \caption{Représentation passive d'une transformation de coordonnées.}
    \label{fig:transf passives}
\end{figure}
\end{enumerate}
...
\end{equation}
Dénotant la paramétrisation locale $\varphi^{-1}_N \equiv \psi_N$, nous allons déduire sa forme. Soit $\varphi_N^i = z^i$. Alors :
\begin{equation}
\label{eq:1 altasS2}
    z^i = \frac{x^i}{1-x^3}
\end{equation}
...
\begin{align*}
    \varphi_S \circ \psi_N :\quad \R^2_0 &\longrightarrow \R^2\\
...
\begin{align}
    \psi_N ( S^2\backslash\{S\} \cap S^2\backslash\{N\}) = \psi_N ( S^2\backslash\{N,S\}) = \R^2_0.
\end{align}
Il s'agit ici d'une subtilité importante dans la définition d'un atlas : il faut être lisse seulement sur l'intersection de leurs domaines (excluant ici $(0,0)$, qui autrement aurait posé problème) :
\begin{exerc}
...
\end{exerc}
\section{Tenseurs revisités}
\label{sec:tenseurs variétés}
Nous devons retravailler la notion de tenseur introduite précédemment dans le cadre d'un espace-temps plat, afin qu'elle soit \emph{intrinsèque} à $\mathcal{M}$ : une variété arbitraire n'est pas un espace vectoriel et un vecteur ne peut donc pas être défini sur l'espace lui-même, il faut passer par les espaces tangents. Heureusement pour nous, la plupart des notions vues au début de ce chapitre se généralisent naturellement à une variété arbitraire. Il subsiste néanmoins quelques différences, dont nous discuterons dans cette section. 
\subsection{Espace tangent}
Fixons un point $m\in \mathcal{M}$ (disons, à $\dim \mathcal{M} = n$) et une courbe $\gamma$ dans $\mathcal{M}$ telle que $\gamma(0)=m$.
\begin{theoremframe}
    \begin{defi}
...
\begin{equation}
    X^i_m f = \left. \frac{\td}{\td \lambda}\right|_{\lambda=0}  f \circ \gamma^i(\lambda).
\end{equation}
...
\begin{align}
    X_mf = \left. \frac{\pd x^\mu}{\pd \lambda} (\gamma(\lambda))\right|_{\lambda = 0} \left. \frac{\pd F}{\pd x^\mu} \right|_{x^\mu=x^\mu(0)}
\end{align}
...
\end{equation}
où les premiers termes sont les composantes du vecteur $X_m$ et les deuxièmes termes $\{\pd_\mu \}_\mu$ forment une base de l'espace $T_m\mathcal{M}$.
\begin{propri}
    L'ensemble des vecteurs $T_m\M$ en un point $m\in \M$ est appelé espace tangent de $\M$ au point $m$. C'est un espace vectoriel et 
    \begin{align}
        \dim (\M) = \dim (T_m\M)
    \end{align}
\end{propri}
...
\end{equation*}
où nous sous-entendons $x^{\mu} = x^{\mu}( \gamma(\lambda))$. Les vecteurs $X = \pd_\mu$ représentent un déplacement tangent à la direction $x^\mu$.
\begin{center}
...
\end{center}
\cutebreak
\subsection{Loi de transformation des vecteurs}
Les vecteurs sont indépendants de la base et du choix de la carte locale. Sous changement général de coordonnées (difféomorphisme) $x^\mu \to x^{\mu'}$, les composantes $X_\mu$ d'un vecteur se transforment selon
    \begin{align}
...
\end{exmp}
\cutebreak
Cherchons à présent la loi de transformation d'un covecteur. Par définition, $w(X) \in \R$ est bien scalaire. Dans une carte locale, on a
\begin{equation}
    w(X) = w_\mu \td x^\mu (X^\nu \pd_\nu ) = w_\mu X^\nu \td x^\mu (\pd_\nu) = w_\mu X^\nu \delta\indices*{^\mu_\nu} = w_\mu X^\mu
\end{equation}
Où nous avons utilisé la linéarité de $\td x^\mu$. Sous transformation générale de coordonnées, on trouve donc via l'invariance de $w(X)$ et la loi de transformation de $X$ que
\begin{equation}
...
En composantes, cette 2-forme s'écrit
\begin{equation}
...
\subsection{Wedge product}
  ...
  Introduisons la notion de \emph{wedge-product}\footnote{Dans certains milieux, également appelé \emph{wesh} product.}, généralisant le produit vectoriel à une dimension quelconque.
\begin{theoremframe}
    \begin{defi}
        Le wedge-product est l'opération 
        \begin{equation}
...
\end{equation} 
Remarquons que par conséquence, il est clair qu'il n'existe pas de p-formes pour $p>\dim \mathcal{M}$.


\subsection{Dérivée extérieur}
L'importance des p-formes différentielles réside notamment dans le fait qu'elles peuvent être intégrées et différenciées sans requérir plus de structure géométrique\footnote{Voir par exemple Carroll ou Nakahara.}. Elles permettent aussi de reformuler certaines lois physiques de manière tensorielle et compacte. Pour un champ lisse de p-formes $\Omega^p$ (donnée d'une p-forme en tout point de la variété), on définit l'opérateur différentiel suivant :
\begin{theoremframe}
    \begin{defi}
...
\end{equation}
\begin{rmk}
    Si $f $ est une 0-forme, alors $\td f$ est la différentielle usuelle :
    \begin{equation} 
    \td f = \frac{\partial f}{\partial x^{\mu}}\td x^{\mu}.
    \end{equation}
\end{rmk}
\cutebreak
L'utilité de la dérivée extérieur réside dans le fait que $\td \omega $ reste un tenseur. Elle est donc valable dans tout système de coordonnées.
\begin{exerc}
...
\end{equation}
les objets de type $\pd_\mu \omega _\nu$ se transforment bien de manière covariante, car le deuxième terme de (\ref{eq: not a tensor}) s'annule nécessairement par linéarité de la transformation. Bien que ce n'est plus le cas sous changement général de coordonnées, la covariance reste assuré lorsqu'on se restreint à des $p$-formes. Par exemple, si une équation permet de se réécrire (dans un référentiel inertiel) sous la forme
\begin{equation}
    \td F = 0
\end{equation}
alors celle-ci reste valable dans tout référentiel. Nous avons ainsi trouvé une manière de généraliser une équation valable en relativité restreinte (espace-temps plat) vers une équation valable dans une variété courbe arbitraire (covariant sous changement général des coordonnées). Néanmoins, celle-ci n'est valable que dans le cas des $p$-formes. Dans le chapitre suivant, nous allons construire une théorie différentielle définie pour tout tenseur : la dérivée covariante.

\subsection{Cohomologie}
Terminons cette section par une introduction informelle à la théorie de cohomologie. Nous invitons le (la) lecteur(ice) intéressé(e) à suivre le cours de Géométrie Différentielle II.
\begin{theoremframe}
    \begin{defi}
