...
\begin{rmk}
    ...
    \begin{enumerate}
        ...
        \item Si la diagonale contient uniquement des +1, alors la métrique est dite \textit{Riemanienne} ou \textit{Euclidienne}.
        \item  Si la métrique s'écrit $(-1, +1, \cdots, +1)$ dans sa forme canonique, elle est dite \textit{Lorentzienne}.
    \end{enumerate}
Comme la métrique définit une forme bilinéaire symétrique, sa signature (en tout point) ne dépend pas du choix des coordonnées. Celle-ci est donc donnée par la métrique dans sa forme canonique.
\begin{theoremframe}
    \begin{defi}
        Une variété Lorentzienne est une variété différentielle lisse munie d'une métrique Lorentzienne notée $(\mathcal{M},g)$. Par défaut, on considèrera le cas quadri-dimensionnel.
    \end{defi}
\end{theoremframe}
\end{rmk}

\section{Système de coordonnées localement inertiel}
D'après le principe d'équivalence, dans les régions suffisamment restreintes de l'espace-temps, on peut annuler les effets de la gravitation en se plaçant dans un référentiel en chute libre (qui est un système de coordonnées localement inertiel). Autrement dit, dans le formalisme de la géométrie différentielle, la métrique doit localement être réduite à celle de la relativité restreinte, la métrique plate de Minkowski $\eta_{\mu\nu}$ (qui décrit la physique en l'absence de gravitation). 
...
\begin{equation}
    4\cdot (4+12+4)=80
\end{equation}
composantes indépendantes, ce qui n'est pas suffisant pour annuler la dérivée seconde de la métrique en toute généralité, bien qu'elle pourrait s'annuler "accidentellement" (voir le théorème \ref{thm:métrique globalement plate} et sa preuve à la section \ref{Preuve: thm4.42} du chapitre 5).
\end{rmk}
...
\begin{align}
    \tilde{\varepsilon}_{{\mu_1} \cdots {\mu_n}} = \left\{
    \begin{array}{cl}
...
\begin{equation}
    \tilde{\varepsilon}_{{\nu_1} \cdots {\nu_n}} \lvert A \rvert = \tilde{\varepsilon}_{{\mu_1} \cdots {\mu_n}} A\indices{^{\mu_1} _{\nu_1}} \cdots A\indices{^{\mu_n} _{\nu_n}}
\end{equation}
Soit en posant $A\indices{^\mu_{\nu'}} = \frac{\pd x^\mu}{\pd x^{\nu'} }$, on trouve
\begin{equation}
    \tilde{\varepsilon}_{{\nu'_1} \cdots {\nu'_n}} = \left| \frac{\pd x^{\mu'}}{\pd x^{\nu}} \right|  \tilde{\varepsilon}_{{\mu_1} \cdots {\mu_n}} \frac{\pd x^{\mu_1}}{\pd x^{\nu_1'} } \cdots \frac{\pd x^{\mu_n}}{\pd x^{\nu'_n} }
\end{equation}
...
\begin{equation}
    \varepsilon_{{\mu_1} \cdots {\mu_n}} = \sqrt{\lvert g\rvert} \tilde{\varepsilon}_{{\nu'_1} \cdots {\nu'_n}}
\end{equation}
...
\begin{equation}
    \varepsilon^{{\mu_1} \cdots {\mu_n}} = \frac{1}{\sqrt{\lvert g\rvert}} \, \tilde{\varepsilon}^{{\nu'_1}  \cdots {\nu'_n}}
\end{equation}
Nous donnerons de plus une identité utile de sommation du tenseur de Levi-Civita :
\begin{equation}
    \varepsilon^{{\mu_1} \cdots {\mu_p} {\alpha_1} \cdots {\alpha_{n-p}}} \varepsilon_{{\mu_1} \cdots {\mu_p} {\beta_1} \cdots {\beta_{n-p}}} = (-1)^s \, p! \, (n-p)! \,\delta^{[\alpha_1}_{\beta_1} \cdots \delta^{\alpha_{n-p}]}_{\beta_{n-p}}
\end{equation}
où $s$ est le nombre de valeurs propres négatives de la métrique ($s=1$ dans le cas Lorentzien). Pour $p = n - 1$ :
\begin{equation}
    \varepsilon\indices{^{\mu_1} ^\cdots ^{\mu_{n-1}} ^{\alpha}} \varepsilon\indices{_{\mu_1} _\cdots _{\mu_{n-1}} _{\beta}}= (-1)^s (n-1) ! \, \delta^\alpha_\beta
\end{equation}
\begin{exerc}
    \label{ex:densités}
    ...
    \end{equation*}
    \begin{enumerate}
        \item Montrez que l'élément infinitésimal
        \begin{equation*}
            \td^n x = \td x^0 \wedge \cdots \wedge \td x^{n-1}
        \end{equation*}
        se transforme comme une densité tensorielle.
        \item Construisez un élément infinitésimal tensoriel (plus précisément, une $n$-forme).
    \end{enumerate}
    L'application
    \begin{equation}
        \int_\mathcal{M} : \Omega^p \to \R : \omega \mapsto \int_\mathcal{M} \omega
    \end{equation}
    se ramène à la définition usuelle d'une intégrale pour la 1-forme $\omega = f(x) \td x$ dans un espace-temps plat unidimensionnel. Le terme $\sqrt{-g}$ présent dans le cas général représente une correction à l'élément infinitésimal, associée à la courbure de la variété.
\end{exerc}
...
\begin{exerc}
    Refaites l'exercice \ref{exercice:maxwell1} dans le formalisme des $p$-formes.
\end{exerc}
...
\end{align}
Comme cette expression est valable pour tout $X$, nous pouvons le supprimer. En multipliant des deux côtés par $\dfrac{\pd x^\alpha}{\pd x^{\beta'}}$, on peut simplifier le premier terme via la règle de la chaîne :
\begin{align}
    \frac{\partial x^{\alpha}}{\partial x^{\beta '}}  \lt \frac{\partial x^{\nu}}{\partial x^{\nu '}} \frac{\pd}{\pd x^\nu} \frac{\partial x^{\mu'}}{ \partial x^{\alpha}} \rt = \frac{\partial x^{\alpha}}{\partial x^{\beta '}}  \lt \frac{\pd}{\pd x^{\nu'}} \frac{\partial x^{\mu'}}{ \partial x^{\alpha}}\rt 
\end{align}
...
\begin{equation}
    T\indices{^\mu_{\!\alpha\beta}} = \Gamma\indices{^\mu_{\![\alpha\beta]}}
\end{equation}
...
\begin{rmk}
    Même sans torsion, les coefficients de connexion ne se transforment pas de manière covariante sous transformation générale des coordonnées : ce ne sont donc toujours pas un tenseur. 
\end{rmk}
...
\begin{align}
    X^{\mu}Y^{\alpha}\partial_{\mu}w_{\alpha} + \cancel{X^{\mu}w_{\alpha}\partial_{\mu}Y^{\alpha}} = (\nabla_X w)_{\alpha}Y^{\alpha} + \cancel{w_{\alpha}X^{\mu}\partial_{\mu}Y^{\alpha}} + w_{\gamma}X^{\mu}\Gamma\indices{^{\gamma}_{\!\mu \alpha}}Y^{\alpha}
\end{align}    
Ceci étant vérifié pour tout $Y$, on trouve une expression de la dérivée covariante pour une 1-forme :
\begin{equation}
...
\begin{theoremframe}
    \begin{defi}
        ...
        Soit une courbe lisse $\gamma:\R \to \mathcal{M}$ à coordonnées $x^{\mu}(\lambda)$ et son champ tangent $U^{\mu} = \frac{\td x^{\mu}}{\td\lambda}$. Un champ de tenseur $T$ est dit \emph{transporté parallèlement} à cette courbe si en tout point de la courbe:
        \begin{equation}
            \nabla_U T = 0
        \end{equation}
    \end{defi}
...
\end{enumerate}
Soit une variété $\mathcal{M}$ munie d'une métrique $g$. Nous allons imposer la \emph{compatibilité métrique} de la connexion affine $\nabla$ avec l'espace métrique $(\mathcal{M},g)$.
\begin{theoremframe}
    \begin{defi}
...
\label{eq:connexion métrique}
    \nabla_U \big(g(X, Y)\big) = g(\nabla_U X, Y) + g(X, \nabla_U Y).
\end{equation}
Ceci n'est rien d'autre qu'une règle de Leibniz pour le produit scalaire. Montrons que l'équation (\ref{eq:connexion métrique}) est équivalente à la 
définition \ref{def:connexion métrique}. Comme $X$ et $Y$ sont transportés parallèlement à $U$, on a $\nabla_U X = 0 = \nabla_U Y$ et donc 
\begin{equation}
    \nabla_U \big(g(X, Y)\big) = 0
\end{equation}
Or, comme $g(X, Y)$ est un scalaire (une 0-forme), par la définition \ref{dérivée covariante 0-forme} :
\begin{equation}
    U^\mu \pd_\mu \big(g(X, Y)\big) = 0
\end{equation}
Comme ceci est vrai pour tout $X,Y$ et $U$, on retrouve bien la définition \ref{def:connexion métrique}.
\begin{exerc}
    Montrez que le transport parallèle préserve la norme d'un vecteur $X$.\footnote{Note : je ne me rappelle pas de ce que je voulais de cet exercice... Rappelez-moi de l'enlever si vous aussi vous trouvez qu'il sert à rien.}
\end{exerc}
\begin{theoremframe}
    \begin{lemme}
...
\begin{theoremframe}
    \begin{theorem}[Théorème fondamental de la géométrie Riemannienne]
        ...
        Soit une variété (pseudo-) Riemannienne munie d'une métrique $(\mathcal{M},g)$. Il existe une unique connexion affine sans torsion et métrique sur $(\mathcal{M},g)$.
    \end{theorem}
\end{theoremframe}  
\begin{proof}
...
\begin{theoremframe}
    ...
    \label{Connexion de Levi-Civita}
        \end{equation}
        sont appelés les \emph{symboles de Christoffel}.
    \end{defi}
\end{theoremframe}
...
\begin{proof}
   Par la propriété \ref{prop: PE}, il existe un système de coordonnée $\{x^{\hat{\mu}}\}$ dans lequel 
    \begin{enumerate}
        ...
        \item $ \left. \partial_{\hat{\alpha}}g_{\hat{\mu}\hat{\nu}} \right|_P=0$
    \end{enumerate}
    Il suit alors immédiatement de la relation (\ref{Connexion de Levi-Civita}) que dans ce référentiel, $ \Gamma\indices{^{\hat{\gamma}}_{\!\hat{\mu} \hat{\nu}}} =0$. 
\end{proof}
...
\end{enumerate}
Illustrons ce procédé avec les équations de Maxwell. En relativité restreinte, i.e. en l'absence de gravitation, les équations de Maxwell s'écrivent
\begin{equation}
    \partial_{\mu}F^{\mu \nu} = J^{\nu}
...
\begin{rmk}
    Justifions cette procédure, qui peut, à première vue, sembler arbitraire. Etant donné un vecteur $A$ dans un certain référentiel, la forme de celui-ci est donné uniquement par la transformation \emph{tensorielle} de celui-ci. Autrement dit, étant donné un objet $A$ dans un certain référentiel, il existe une unique généralisation covariante, c'est-à-dire qui se transforme de manière tensorielle et qui se réduit à $A$ dans le référentiel donné.
\end{rmk}
\begin{rmk}
    Nuançons à présent la procédure présentée. Celle-ci ne permet en effet uniquement à la généralisation d'une seule dérivée covariante :
    \begin{align}
        \pd_\mu \pd_\nu \:\cancel{\to}\: \nabla_\mu \nabla_\nu
    \end{align}
    en raison de termes de dérivées secondes en la métrique apparaissant dans le terme de droite. Ceux-ci ne pourront donc en général pas être annulés, même dans un RLI. La raison pourquoi les équations de Maxwell sous la forme \ref{équation de Mawell vallablle dans tout les réf} sont bien tensorielles, alors que $F_{\mu\nu}$ comporte un autre jeu de dérivées, est que la combinaison précise donne bien un tenseur $F_{\mu\nu}$ (en vertu d'être une $2$-forme).
\end{rmk}
%\subsection{Interprétation géométrique de la torsion*}
%\href{https://youtu.be/nJpONHO_X5o?si=w-2czEDmyKMmQp6x}{WIP}
%\section{Sur le lien entre la dérivée covariante et la dérivée extérieure*}
%\href{https://youtu.be/SfOiOPuS2_U?si=J1I-q-oWcudEWOLY}{WIP}
\section{Courbure}
...
\begin{exmp}
    \href{https://youtu.be/Af9JUiQtV1k?si=aQAsikQre4qSr2fq}{Transport parallèle sur la sphère.}
\end{exmp}
...
\begin{align*}
    ...
    &= X^{\alpha}\partial_{\alpha}Y^{\mu}\partial_{\mu}f + \cancel{X^{\alpha}Y^{\mu}\partial_{\alpha}\partial_{\mu}f} - Y^{\alpha}\partial_{\alpha}X^{\mu}\partial_{\mu}f - \cancel{Y^{\alpha}X^{\mu}\partial_{\alpha}\partial_{\mu}f}\\
...
\end{align}
Comme $\nabla_\mu \pd_\nu$ est un vecteur, l'expression se développe davantage d'après par la définition de ses composantes (\ref{def: dérivée covariante vecteur}) :
\begin{align}
...
\begin{theoremframe}
    \begin{theorem}
    \label{thm:métrique globalement plate}
...
\begin{theoremframe}
    \begin{propri}
        ...
        \begin{enumerate}
            ...
            \item Conservé (identité de Bianchi) : $\nabla_{\mu}G\indices{^{\mu\nu}} = 0$
        \end{enumerate}
    \end{propri}
\end{theoremframe}
\begin{proof}
