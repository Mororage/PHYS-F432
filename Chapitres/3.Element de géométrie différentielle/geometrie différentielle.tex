...
\begin{theoremframe}
    ...
    \end{notat}
\end{theoremframe}
La définition (mathématiquement) précise d'un espace tangent ne pourra être donné qu'après avoir introduit la notion de variété différentielle. 
\begin{rmk}
    La notion locale (au point $P$) de cette définition sera importante : un vecteur ne pourra pas être vu comme une flèche rejoignant le point $P$ à un autre point $Q \in \mathcal{M}$. Le vecteur \textit{ne vit pas} sur le même espace que les points $P$ et $Q$. Lorsque $\mathcal{M} = \R^{1,3}$, il n'y a pas de problème car les ensembles $\mathcal{M}$ et $T_P\mathcal{M}$ coincident (en vertu du fait que $\R^{1,3}$ est lui-même un espace vectoriel) : on peut alors les voir comme des flèches rejoignant deux points distincts. Ce ne sera plus le cas si l'espace-temps est courbe.
\end{rmk}
\begin{theoremframe}
    \begin{defi}
...
\end{equation}
où les premiers termes sont les composantes du vecteur $X_m$ et les deuxièmes termes $\{\pd_\mu \}_\mu$ forment une base de l'espace $T_m\mathcal{M}$. Remarquons que la dimension de $T_m\mathcal{M}$ est $n$ (la même que l'espace $\mathcal{M}$ correspondant).
\begin{center}
