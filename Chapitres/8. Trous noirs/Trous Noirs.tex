...
\end{align}
À $r$ fixé, c'est l'équation de deux demi-droites de pentes $\lt 1- \frac{2m}{r}\rt$. De plus, on peut analyser les cas asymptotiques suivants :
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Chapitres/8. Trous noirs/Images/Cônes Schwarzschild.png}
    \caption{Les cônes de lumières dans le plan $(t,r)$}
    \label{fig: cônes de lumières}
\end{figure}
\begin{rmk}
...
\begin{align}
    a^\alpha (\nabla_U U)^\alpha &= U^\beta \nabla_\beta U^\alpha = U^\beta \lt \pd_\beta U^\alpha + \Gamma\indices{^\alpha_{\!\beta \gamma}} \,U^\gamma \rt\\
    &= U^0 \pd_0 U^\alpha + U^0 \, \Gamma\indices{^\alpha_{00}} \, U^0\\
    &= \lt 1 - \frac{2m}{r} \rt^{-1} \Gamma\indices{^\alpha_{00}}
\end{align}
Le seul symbole de Christoffel $\Gamma\indices{^\alpha_{00}}$ non-nul pour la métrique de Schwarzschild est 
\begin{equation}
    \Gamma\indices{^1_{00}} = \frac{m}{r^2} \lt 1 - \frac{2m}{r} \rt 
\end{equation}
Ce qui nous donne une accélération de
\begin{equation}
    a^\alpha = (0, \frac{m}{r^2},0,0)
\end{equation}
Pour maintenir un objet de masse $M$ statique dans un champ de gravitation de Schwarzschild, il faut lui appliquer une force $F = M \frac{m}{r^2}$ (ou de manière équivalente une accélération $a = \frac{m}{r^2}$) dans la direction radiale pour compenser l'attraction gravitationnelle (résultat newtonien). Néanmoins, cette expression de l'accélération n'est valable que dans le référentiel propre de l'objet. Une quantité invariante est la norme de ce vecteur : 
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Chapitres/8. Trous noirs/Images/trous noir signaux vers extérieur.png}
    \caption{Chute dans un trou noir depuis un observateur lointain : il ne voit jamais l'objet franchir l'horizon.}
    \label{fig:signaux venant d'un trou noir}
\end{figure}
...
\end{equation}
Prenons un objet tombant radialement ($\dot{\varphi} = 0 = \dot \theta$) en chute libre dans la région $r=2m$. On obtient alors $a=0$ par la constante du mouvement de l'équation des géodésiques (\ref{géodésiques de Schwarzschild}). Les autres constantes du mouvement donnent :
\begin{align}
...
\begin{align}
    \frac{\td r}{\td t} &= - b^{-1} \, \frac{r - 2m}{r} b + \mathcal{O}\lt\lt\frac{r-2m}{r}\rt^2\rt \simeq - \frac{r-2m}{2m}\\
...
\end{align}
L'observateur en chute libre atteint donc $r=2m$ en un temps (propre) fini.
\begin{equation}
    \boxed{\tau (r) = \frac{1}{3} \sqrt{\frac{2}{m}} \lt r_0^{3/2} -r^{3/2}\rt}
\end{equation}
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{Chapitres/8. Trous noirs/Images/temps propre chute.png}
    \caption{Chute dans un trou noir dans le référentiel propre : on atteint l'horizon en un temps propre fini.}
    \label{fig:chute propre dans trou noir}
\end{figure}
...
Nous avions vu que les géodésiques radiales nulles étaient données par l'équation (\ref{eq:géodésiques radiales nulles}) :
\begin{equation}
...
\end{equation}
Cette coordonnée est appelée \emph{tortoise coordinate}. Le changement de coordonnées n'est valable que pour $r>2m$. Dans ces coordonnées, la métrique s'écrit
\begin{equation}
...
\begin{equation}
    \td t^2 = \pm \td r^{*2}
\end{equation}
qui se comportent bien dans ces coordonnées et ont une ouverture constante. 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Chapitres/8. Trous noirs/Images/cônes tortoise.png}
    \caption{Les cônes de lumière selon la \emph{tortoise coordinate}}
    \label{fig:cônes tortoise}
\end{figure}
Par contre, bien qu'aucune composante de la métrique ne diverge en $r=2m$, la métrique est toujours dégénérée en cette région qui se trouve à présent en $r^* = -\infty$. 
...
\begin{equation}
    t = \pm r^* + \text{cste}
\end{equation}
On remarque que si le signe est choisi positivement, alors le rayon augmente avec le temps : il s'agit d'une géodésique sortante. Inversément, un signe négatif donne lieu à une géodésique entrante. On pose donc :
\begin{equation}
    \begin{dcases}
        v = t + r^* \\
        u = t - r^*
...
\begin{rmk}
    Dans ces coordonnées, les courbes orientés futur (ayant un temps $t$ croissant) sont à $u$ et $v$ croissants.
\end{rmk}
...
\begin{align}
    ...
    & = - \lt 1 - \frac{2m}{r} \rt \lt \td v^2 - - \frac{2 \, \td v \td r}{1 - \frac{2m}{r}} + \cancel{\frac{\td r^2}{\lt 1 - \frac{2m}{r} \rt^2}} \rt + \cancel{\frac{\td r^2}{1 - \frac{2m}{r}}} + r^2 \td \Omega^2
\end{align}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Chapitres/8. Trous noirs/Images/cônes EF entrantes.png}
    \caption{Les cônes de lumière dans les coordonnées d'Eddington-Finkelstein entrantes. La ligne horizontale $v =$ cste correspond aux rayons lumineux entrants, donc le bord du cône de lumière.}
    \label{fig:cônes EF entrantes}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Chapitres/8. Trous noirs/Images/cônes EF sortantes.png}
    \caption{Les cônes de lumière dans les coordonnées d'EF sortantes.}
    \label{fig:cônes EF sortantes}
\end{figure}
...
\begin{itemize}
    \item $v = $cste, on a $t \to + \infty$ : c'est ce qu'on appellera la \emph{région} \RNum{2}.
    \item $u = $cste, on a $t \to - \infty$ : c'est ce qu'on appellera la \emph{région} \RNum{3}.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Chapitres/8. Trous noirs/Images/KS1.png}
    \caption{La région \RNum{1} dans le demi-plan $(\mathfrak{u},\mathfrak{v})$.}
    \label{fig:KS1}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Chapitres/8. Trous noirs/Images/KS2.png}
    \caption{La région \RNum{2} dans le demi-plan $(\mathfrak{u},\mathfrak{v})$.}
    \label{fig:KS2}
\end{figure}
...
Exprimons la métrique dans les coordonnées $(\mathfrak{u},\mathfrak{v},\theta,\varphi)$. Par la relation (\ref{coordonnées KS 3}) :
\begin{align}
...
Dans le plan $(\mathfrak{u},\mathfrak{v})$, les rayons lumineux sont donnés par $\td \mathfrak{u}^2 = \td \mathfrak{v}^2$ donc $\td \mathfrak{u} = \pm \td \mathfrak{v}$ comme pour la métrique de Minkowski à 1+1 dimensions ! La métrique y est conformément plate (i.e. plate à un facteur près). Cette propriété est très utile pour étudier la structure causale de l'espace-temps.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Chapitres/8. Trous noirs/Images/KS3.png}
    \caption{La structure globale de la métrique de Schwarzschild. Les lignes en zig-zag correspondent à la région $r=0$.}
    \label{fig:KS3}
\end{figure}
...
\begin{rmk}
    On peut se rendre compte de l'existence de \RNum{4} en étudiant des géodésiques spatiales ($\td s^2 >0$, $v>c$) et en demandant que celles-ci soient complètes, i.e. existent pour toute valeur du paramètre affin de la géodésique. La région \RNum{4} est une autre région asymptotiquement plate (comme un autre Univers), mais aucun observateur physique ne peut y accéder. Elle est connectée à \RNum{1} par le \emph{Einstein-Rosen bridge}.
\end{rmk}
\begin{rmk}
...
\end{rmk}
\begin{rmk}
    L'union de ces régions, \RNum{1} $\cup$ \RNum{2} $\cup$ \RNum{3} $\cup$ \RNum{4} est l'extension maximale de la géométrie de Schwarzschild, ayant une topologie $\R^2 \times S^2$ (soit $(\mathfrak{u},\mathfrak{v}) \times (\theta,\varphi) \sim 2 \times$ Schwarzschild pour laquelle $r\in \R^+_0$).
\end{rmk}
\subsection{Horizon et trou noir}
Un observateur en orbite circulaire (donc $r>2m$) dans la géométrie de Kruskal-Szerekes ressemble fortement à l'observateur accéléré en espace-temps plat (l'observateur de Rindler). \RNum{1} $\cup$ \RNum{2} est son futur causal $\mathcal{J}^+(\mathcal{S})$, l'ensemble des évènements qu'il peut influencer. Le bord de $\mathcal{J}^+(\mathcal{S})$ est l'horizon passé pour $\mathcal{S}$. \\
Considérons à présent un évènement $O$ dans la région \RNum{2} : son futur est complètement limité par la singularité. Toute courbe causale passant par $O$ aboutit nécessairement à la singularité $r=0$ où règnent des forces de marée infinies (mesurée par le scalaire de Kretschmann $K \propto \frac{1}{r^6} \to \infty$ lorsque $r\to 0$). Rien ne peut s'échapper de la région \RNum{2} vers l'infini (i.e. retourner dans la région \RNum{1}). Un observateur dans \RNum{1} n'a aucun moyen de savoir ce qui se passe dans \RNum{2} (aucun signal ne peut lui parvenir de cette région), en particulier il ne peut pas \emph{voir} la singularité future $r=0$. 
\begin{theoremframe}
    \begin{defi}
        La région $r<2m$ définit un \emph{trou noir}, c'est-à-dire une région de l'espace-temps causalement déconnectée de la région asymptotique $r \to \infty$. La surface limite \emph{de non-retour} $r=2m$ est appellée \emph{horizon des évènements}.
    \end{defi}
\end{theoremframe}
Notons que l'horizon des évènements d'un trou noir, contrairement à l'horizon de Rindler, est une propriété de l'espace-temps lui-même, et pas limité à une classe d'observateurs particuliers (comme les observateurs uniforméments accélérés dans le cas de Rindler).
\section{Effondrement gravitationnel et formation des trous noirs}
La structure globale de la géométrie de Schwarzschild nous a permis de définir la notion de trou noir. Elle a également mis en évidence des régions encore plus surprenantes : un \emph{trou blanc} (région \RNum{3}) et un \emph{univers miroir} (région \RNum{4}). L'existence de ces deux dernières régions nécessite un trou noir éternel (en particulier, éternel dans le passé) dans une région asymptotiquement plate, ce qui n'est pas très réaliste. Bien qu'on ne puisse pas les exclure à strictement parler, on n'a pas d'évidence pour leur existence. Si ceci représentait également la seule façon d'obtenir des \emph{trous noirs}, on pourrait alors sans doute simplement les considérer comme des curiosités mathématiques, sans réalité physique, ce qui fut d'ailleurs longtemps le cas. Néanmoins, il est désormais admis que des trous noirs non-éternels existent, et que ceux-ci peuvent notamment se former par effondrement gravitationnel d'une étoile ayant épuisé son combustible nucléaire. Nous allons évoquer certains de ces aspects de manière sommaire.
...
La plupart des objets astrophysiques ne vont pas s'effondrer jusqu'à $R = \frac{9}{4} M$ et former un trou noir. Une planète ordinaire, supportée par des pressions matérielles, existe essentiellement pour toujours. Mais le destin des étoiles, en particulier les étoiles massives, est une autre histoire.

Sans entrer dans les détails de la description de l'effondrement gravitationnel ou de l'évolution stellaire (qui est laissé aux cours d'astrophysique) on peut mentionner les stades d'évolution suivants. Du cours de son existence, une étoile va évoluer en brûlant ses constituants nucléaires en étapes successives et commençant par les éléments les plus légers. Notre Soleil par exemple est en phase de combustion d'Hydrogène, phase appelée \emph{séquence principale}. La chaleur/rayonnement produit par la fusion d'éléments légers en éléments plus lourds donne lieu à une pression de rayonnement qui contrebalance l'attraction gravitationnelle. 

Lorsque ces processus de fusion se terminent, l'étoile finit par s'effondrer sous sa propre attraction gravitationnelle. Ceci peut être arrêté par la \emph{pression de dégénérescence des électrons} qui se manifeste lorsque les densités et pressions mises en jeu deviennent très élevées : la matière se comporte comme un gaz quantique dégénéré et le principe d'exclusion se manifeste au niveau macroscopique; les électrons s'opposent à se trouver dans le même état quantique ce qui implique une répulsion entre atomes voisins. Pour une étoile ayant une masse $M< 1.4 M_\odot$ en fin de vie (limite de Chandrasekkar)\footnote{L'étoile peut néanmoins être beaucoup plus massif en début de vie, comme une partie de sa masse sera éjectée en cours de vie.}, l'effondrement s'arrête et l'astre résultant est une \emph{naine blanche}. La densité d'une telle étoile est de l'ordre de $\rho \sim 1000$kg/cm$^3$, à comparer avec la densité de l'eau $\rho \sim 1$g/cm$^3$ et celle du Soleil $\rho \sim 150$g/cm$^3$.\\
Si par contre $M> 1.4 M_\odot$, l'effondrement continue et les conditions deviennent telles que la désintégration $\beta$ inverse (p$^+$ + e$^- \to$ n + $\nu_e$) est favorisée et une \emph{étoile à neutrons} se forme, dans laquelle la pression de dégénérescence des neutrons peut contrebalancer la force gravitationnelle tant que $M \lesssim 3 M_\odot$ (limite d'Oppenheimer-Volkar). Une telle étoile possède un rayon de l'ordre des $10$km (Bruxelles), une densité de $\rho \sim 10^{12}$kg/cm$^3$ et une masse de $2-3M_\odot$.
\begin{rmk}
...
LIGO : première détection de la fusion de deux trous noirs le 14 septembre 2015 (\textsc{GW150914}) à masses $36 M_\odot + 29M_\odot \to 62 M_\odot$ plus ondes gravitationnelles. Prix Nobel en 2017 pour Weiss, Barish et Thorne.
...
\begin{enumerate}
    ...
    \end{equation}
    La relativité générale cesse donc d'être valide dans le régime $r\to 0$. En comparaison, on peut décider de modéliser un objet comme ponctuel (qui est une forme de singularité), mais il s'agit simplement d'une modélisation mathématique : on néglige le volume d'un électron car on peut négliger ses effets. Les trous noirs sont différents : la formation d'un trou noir/singularité est \emph{générique} (cf. théorèmes de singularité), et la singularité est une prédiction (et non une modélisation).
